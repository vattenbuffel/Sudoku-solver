...
def convert_to_noa_format(soduko):
    soduko_ = []
    for cell in soduko:
        soduko_.append(cell.returnSolved())
    
    import numpy as np
    return np.array(soduko_).reshape(-1,9)

def generate_soduko(level):
    ...
    if level == 'Easy':
        ...
        s = puzzleGen(p)
        if s[2] != 'Easy':
            return generate_soduko(level)
        t2 = time.time()
        t3 = t2 - t1
        #print("Runtime is " + str(t3) + " seconds")
        #print("Guesses: " + str(s[1]))
        print("Level: " + str(s[2]))
        return convert_to_noa_format(s[0])
